...
def dane_to_predict():
    currency = 'btc'
    start_date = 1569880800
    time_difference = 1569880800 - 1556661600
    nr_of_results = int(time_difference / (60 * 60 * 24))
    end_date = start_date + (nr_of_results*60*60*24)

    link = "https://www.bitstamp.net/api/v2/ohlc/" + currency + "usd/"
    bitstamp = requests.get(link,
                            params={"step": 86400, "limit": nr_of_results, "start": start_date, "end": end_date})
...
with f:
    ...
    writer = csv.DictWriter(f, fieldnames=fnames)
    writer.writeheader()
    for i in range(len(new_volumes_average)):
...
real_volumes, real_timestamps = dane_to_predict()

f = open('real_volumes.csv', 'w')
with f:
    fnames = ['timestamp', 'volume']
    writer = csv.DictWriter(f, fieldnames=fnames)
    writer.writeheader()
    for i in range(len(real_volumes)):
        writer.writerow({'timestamp': real_timestamps[i], 'volume': real_volumes[i]})